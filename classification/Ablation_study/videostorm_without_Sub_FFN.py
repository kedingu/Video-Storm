"""
Video-STORM Ablation A: full architecture without FFN in DynamicSTORMBlock.

Ablation target: the sub-stage FFN (pwconv1 + act + pwconv2) inside every
DynamicSTORMBlock is removed. All other components are preserved:
  - AdaptiveTemporalConv in all stage blocks
  - DynamicSTORMBlock with concatenation-fusion, ContMix, spatio-temporal LEPE,
    gate+SE, per-channel alpha/beta blending, proj->split context update
  - RetroActiveBridge between sub-stage s2 and s3

This isolates the contribution of the sub-stage FFN, which is the most
parameter-heavy component of DynamicSTORMBlock (operates on out_dim = dim+ctx_ch).
...
Pipeline (frame-wise on B*T frames):
    ...
    sub_blocks_s2 x sub_depth[0]  DynamicSTORMBlock no-FFN (dim=dims[2])
    patch_embedx   RetroActiveBridge  (dims[2] -> dims[3], stride=2)
    sub_blocks_s3 x sub_depth[1]  DynamicSTORMBlock no-FFN (dim=dims[3])
...
class UniRepLKNetBlock(nn.Module):
    ...
    When num_frames > 0, an AdaptiveTemporalConv branch is fused with the spatial
    features via parameter-free RMSNorm before the shared SE.
...
class AdaptiveTemporalConv(nn.Module):
    ...
    Dilation schedule adapts to num_frames via geometric progression plus a
    terminal branch (88% at T=8, 94% at T=16, 97% at T=32, 98% at T=64).
...
class DynamicSTORMBlock(nn.Module):
    """DynamicSTORMBlock without sub-stage FFN (Ablation A).

    Identical to the full DynamicSTORMBlock except pwconv1 + act + pwconv2
    are removed. The forward path is:
      context blending -> concat-fusion -> ContMix -> ST-LEPE -> gate+SE
      -> residual -> proj -> split (context update)
    The proj still outputs out_dim = dim+ctx_ch for non-last blocks so the
    split-based context update is preserved at zero additional cost.
    ...
    def __init__(self, dim, kernel_size, ctx_ch, smk_size=5, num_heads=4,
                 ...
                 is_first=False, is_last=False, num_frames=0):
        super().__init__()
        ...
        # proj -> out_dim (non-last) or dim (last) -- FFN REMOVED, proj feeds split directly
        if not is_last:
    ...
    def _forward_inner(self, x, h_x, h_r):
        ...
        # ContMix -- dual-scale neighbourhood attention
...
# ---------------------------------------------------------------------------
# VideoSTORM -- Ablation A
# ---------------------------------------------------------------------------

class VideoSTORM(nn.Module):
    """Video-STORM Ablation A: full architecture without FFN in DynamicSTORMBlock.
    All other components (AdaptiveTemporalConv, context flow, ContMix, ST-LEPE,
    RetroActiveBridge) are preserved. Requires: natten, einops."""
    ...
    def __init__(self, in_chans=3, num_classes=1000,
                 ...
                 sub_drop_path_rate=0., projection=2048,
                 **kwargs):
        super().__init__()
        ...
        # Note: sub_ffn_factor is intentionally absent -- no FFN in DynamicSTORMBlock
    ...
    def load_pretrained_2d(self, ckpt, strict=False, skip_head=True,
                            map_location='cpu'):
        """Load a 2D ImageNet checkpoint into VideoSTORM Ablation A.
...
def videostorm_s(pretrained_2d=None, pretrained_2d_strict=False, **kwargs):
    """Video-STORM-S Ablation A: without FFN in DynamicSTORMBlock. Requires: natten, einops."""
...
def videostorm_b(pretrained_2d=None, pretrained_2d_strict=False, **kwargs):
    """Video-STORM-B Ablation A: without FFN in DynamicSTORMBlock. Requires: natten, einops."""
    _pop_timm_args(kwargs)
    kwargs.setdefault('sub_depth',     (4, 2))
    kwargs.setdefault('sub_num_heads', (4, 8))
    kwargs.setdefault('smk_size',      5)
...
def videostorm_l(pretrained_2d=None, pretrained_2d_strict=False, **kwargs):
    """Video-STORM-L Ablation A: without FFN in DynamicSTORMBlock. Requires: natten, einops."""
    _pop_timm_args(kwargs)
    kwargs.setdefault('sub_depth',     (4, 2))
    kwargs.setdefault('sub_num_heads', (4, 8))
    kwargs.setdefault('smk_size',      5)
