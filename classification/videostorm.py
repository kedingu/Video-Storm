...
Spatio-temporal modeling (all stages, k != 0):
    ...
    Fusion via direct addition before the shared SE recalibration.
...
Training:
    BF16 (bfloat16) on H100 via autocast(dtype=torch.bfloat16) WITHOUT GradScaler.
    BF16 range (~3.4e38) eliminates overflow -- no BN normalization or temporal
    scaling workarounds needed. IN-22K checkpoint gammas loaded as-is.
"""
import torch
...
class GRNwithNHWC(nn.Module):
    """GRN (Global Response Normalization) [NHWC]
    """
    def __init__(self, dim, use_bias=True):
        super().__init__()
        self.use_bias = use_bias
...
class UniRepLKNetBlock(nn.Module):
    ...
    an AdaptiveTemporalConv branch is fused with the spatial features via direct
    addition before the shared SE, enabling spatio-temporal recalibration at every
    scale. BF16 training eliminates overflow -- no scaling workaround needed."""
    def __init__(self, dim, kernel_size, drop_path=0.,
        ...
        # Not created for k=0 or deploy mode.
    ...
    def compute_residual(self, x):
        ...
        if self.temporal_branch is not None:
            ...
            y = self.se(v_s + v_t)  # direct addition -- BF16 safe
        else:
            y = self.se(v_s)
...
class DynamicSTORMBlock(nn.Module):
    ...
    spatial and adaptive temporal branches via direct addition. Per-channel
    ...
    out_dim = dim+ctx_ch. BF16 training eliminates overflow -- no scaling needed."""
    def __init__(self, dim, kernel_size, ctx_ch, smk_size=5, num_heads=4,
        ...
        # Temporal LEPE -- direct addition with spatial, BF16 safe
        self.t_lepe = None
    ...
    def _forward_inner(self, x, h_x, h_r):
        ...
        # Spatio-temporal LEPE -- direct addition, BF16 safe
        lepe_s = self.norm(self.dwconv(x))
        if self.t_lepe is not None:
            lepe = lepe_s + self.t_lepe(x)  # direct addition
        else:
            lepe = lepe_s
...
class VideoSTORM(nn.Module):
    ...
    Training: use autocast(dtype=torch.bfloat16) WITHOUT GradScaler on H100.
    ...
    def load_pretrained_2d(self, ckpt, strict=False, skip_head=True,
        ...
        IN-22K checkpoint gammas are loaded as-is (BF16 training eliminates overflow).
        Temporal and sub-stage keys are absent from ImageNet checkpoints and
        initialised from scratch via strict=False. For video resume checkpoints,
        all weights are preserved. DDP prefix stripped automatically."""
        def _load(p):
...
def videostorm_s(pretrained_2d=None, pretrained_2d_strict=False, **kwargs):
    ...
    Train with autocast(dtype=torch.bfloat16) on H100. No GradScaler needed.
    Requires: natten, einops."""
