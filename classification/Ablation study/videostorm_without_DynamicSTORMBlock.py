"""
Video-STORM Ablation B: AdaptiveTemporalConv only, no sub-stages.

Ablation target: the entire sub-stage pipeline is removed:
  - context_encoder, RetroActiveBridge, sub_blocks_s2, sub_blocks_s3
  - DynamicSTORMBlock (and all associated context flow machinery)

Preserved:
  - AdaptiveTemporalConv in all stage blocks (k != 0)
  - Standard 4-stage backbone with spatio-temporal fusion via RMSNorm+SE
  - video_head fed directly from stage-3 output (dims[3] channels)

This isolates the contribution of the sub-stage dynamic modulation with
context flow, by comparing against a model that has temporal modeling
but no overarching retro-active context guidance.

Pipeline (frame-wise on B*T frames):
    downsample_layers[0-1] + stages[0-1]  -- low/mid-level ST features
    downsample_layers[2]   + stages[2]    -- mid-level ST features
    downsample_layers[3]   + stages[3]    -- high-level ST features (dims[3])
...
# ---------------------------------------------------------------------------
# AdaptiveTemporalConv -- preserved from full VideoSTORM
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# VideoSTORM -- Ablation B
# ---------------------------------------------------------------------------

class VideoSTORM(nn.Module):
    """Video-STORM Ablation B: AdaptiveTemporalConv only, no sub-stage modulation.
    Four-stage backbone with spatio-temporal fusion at every block (k != 0).
    No context_encoder, RetroActiveBridge, or DynamicSTORMBlock.
    video_head fed directly from stage-3 output."""
    ...
    def __init__(self, in_chans=3, num_classes=1000,
        ...
        # Stages 0-3 -- AdaptiveTemporalConv active for all k != 0
        ...
        # Classification head -- directly from stage-3 (no sub-stages)
    ...
    def _forward_features_2d(self, x):
        # Linear 4-stage pipeline -- no sub-stages
        for i in range(4):
    ...
    def load_pretrained_2d(self, ckpt, strict=False, skip_head=True,
                            map_location='cpu'):
        """Load a 2D ImageNet checkpoint into VideoSTORM Ablation B."""
        def _load(p):
...
def videostorm_s(pretrained_2d=None, pretrained_2d_strict=False, **kwargs):
    """Video-STORM-S Ablation B: AdaptiveTemporalConv only, no sub-stages."""
    _pop_timm_args(kwargs)
    # Remove sub-stage specific kwargs if passed accidentally
    for k in ('sub_depth', 'sub_num_heads', 'smk_size', 'sub_ffn_factor'):
        kwargs.pop(k, None)
...
def videostorm_b(pretrained_2d=None, pretrained_2d_strict=False, **kwargs):
    """Video-STORM-B Ablation B: AdaptiveTemporalConv only, no sub-stages."""
    _pop_timm_args(kwargs)
    for k in ('sub_depth', 'sub_num_heads', 'smk_size', 'sub_ffn_factor'):
        kwargs.pop(k, None)
    kwargs.setdefault('projection', 2048)
...
def videostorm_l(pretrained_2d=None, pretrained_2d_strict=False, **kwargs):
    """Video-STORM-L Ablation B: AdaptiveTemporalConv only, no sub-stages."""
    _pop_timm_args(kwargs)
    for k in ('sub_depth', 'sub_num_heads', 'smk_size', 'sub_ffn_factor'):
        kwargs.pop(k, None)
    kwargs.setdefault('projection', 2048)
